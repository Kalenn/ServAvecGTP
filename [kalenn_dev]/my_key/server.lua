            ...
            if #result == 0 then
                MySQL.Async.execute('INSERT INTO user_key (identifier, plate) VALUES (@identifier, @plate)', {
                    ...
                    if rowsChanged > 0 then
                        TriggerClientEvent('esx:showNotification', _source, 'Vous avez reçu une clé pour le véhicule avec la plaque ' .. plate)
                        -- Ajoute la clé au trousseau de clé dans l'inventaire
                        xPlayer.addInventoryItem('vehicle_key', 1)
                    end
                end)
            else
                TriggerClientEvent('esx:showNotification', _source, 'Vous possédez déjà une clé pour ce véhicule.')
            end
        end)
    else
        print(('esx_garage:onVehicleOut: xPlayer is nil for source %s'):format(_source))
    end
end)
...
AddEventHandler('esx_garage:pullOutVehicle', function(vehicleProps)
    ...
    if xPlayer then
        ...
        print(('esx_garage:pullOutVehicle: xPlayer is nil for source %s'):format(_source))
    end
end)
...
AddEventHandler('esx_garage:updateOwnedVehicle', function(stored, parking, Impound, data, spawn)
    ...
    if xPlayer then
        ...
        print(('esx_garage:updateOwnedVehicle: xPlayer is nil for source %s'):format(_source))
    end
end)
...
AddEventHandler('esx_garage:setImpound', function(Impound, vehicleProps)
    ...
    if xPlayer then
        ...
        print(('esx_garage:setImpound: xPlayer is nil for source %s'):format(_source))
    end
end)

ESX.RegisterServerCallback('esx_vehicleshop:getPlayerKeys', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer then
        MySQL.Async.fetchAll('SELECT plate FROM user_key WHERE identifier = @identifier', {
            ['@identifier'] = xPlayer.identifier
        }, function(result)
            local keys = {}
            for i=1, #result, 1 do
                table.insert(keys, result[i].plate)
            end
            cb(keys)
        end)
    else
        print(('esx_vehicleshop:getPlayerKeys: xPlayer is nil for source %s'):format(source))
    end
end)
...
ESX.RegisterServerCallback('esx_garage:getVehiclesInParking', function(source, cb, parking)
    ...
    if xPlayer then
        ...
        print(('esx_garage:getVehiclesInParking: xPlayer is nil for source %s'):format(source))
    end
end)
...
ESX.RegisterServerCallback('esx_garage:getVehiclesToPound', function(source, cb)
    ...
    if xPlayer then
        ...
        print(('esx_garage:getVehiclesToPound: xPlayer is nil for source %s'):format(source))
    end
end)
